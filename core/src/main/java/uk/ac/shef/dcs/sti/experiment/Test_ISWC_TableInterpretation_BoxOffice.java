...
import uk.ac.shef.dcs.sti.algorithm.tm.selector.CellSelector;
import uk.ac.shef.dcs.sti.algorithm.tm.selector.LTableContentRow_Sampler_nonEmpty;
import uk.ac.shef.dcs.sti.algorithm.tm.selector.Sampler_LTableContentCell_OSPD_nonEmpty;
...
import uk.ac.shef.dcs.sti.xtractor.TableXtractorBoxOffice;
 ...
 */
public class Test_ISWC_TableInterpretation_BoxOffice {
    private static Logger log = Logger.getLogger(Test_ISWC_TableInterpretation_BoxOffice.class.getName());

    public static int[] IGNORE_COLUMNS = new int[]{1};
        ...
        for (File f : all) {
            ...
            try {
                ...
                if (!complete) {
                    ...
                    try {
                        missedWriter = new PrintWriter(new FileWriter("ti_boxoffice(iswc)_missed.csv", true));
                ...
                PrintWriter missedWriter = null;
                try {
                    missedWriter = new PrintWriter(new FileWriter("ti_boxoffice_missed(iswc).csv", true));
                    missedWriter.println(count + "," + inFile);
                    missedWriter.close();
                } catch (IOException e1) {
                    e1.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
    ...
    public static boolean process(MainInterpreter_and_Forward_learner interpreter, LTable table, String sourceTableFile, LTableAnnotationWriter writer,
        ...
        try {
            ...
            write_iswc_output(table,annotations,new File(outFilename).getName(), "film-boxoffice-genre.txt.tableminer");
    ...
    public static void write_iswc_output(LTable table, LTableAnnotation tableAnnotation, String imdb_file_name, String outFile) throws IOException {
        ...
        if (has != null && has.size() > 0) {
            ...
            for (HeaderAnnotation ha : has) {
                if (ha.getAnnotation_url().startsWith("/film/film_genre")
                        ||ha.getAnnotation_url().equals("/base/americancomedy/comedy_genre")
                        ||ha.getAnnotation_url().equals("/base/animemanga/anime_manga_genre")
                        ||ha.getAnnotation_url().equals("/broadcast/genre")
                        ||ha.getAnnotation_url().equals("/comedy/comedy_genre")
                        ||ha.getAnnotation_url().equals("/tv/tv_genre")
                        ) {
                    correct=true;
                    break;
                }

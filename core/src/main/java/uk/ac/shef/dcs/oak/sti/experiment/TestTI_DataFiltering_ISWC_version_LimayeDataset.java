...
import uk.ac.shef.dcs.oak.sti.algorithm.tm.selector.*;
...
import uk.ac.shef.dcs.oak.sti.xtractor.TableNormalizerDummy;
 ...
 */
public class TestTI_DataFiltering_ISWC_version_LimayeDataset {
    private static Logger log = Logger.getLogger(TestTI_DataFiltering_ISWC_version_LimayeDataset.class.getName());
...
/*        freebaseMatcher.find_typesForEntityId("/m/02hrh1q");
        ...
        Disambiguator disambiguator = new Disambiguator(
                freebaseMatcher,
                new DisambiguationScorer_TM_ISWC_version(
                        stopWords,
        ...
        if (cellSelector == 0) {
            ...
            selector=new Sampler_LTableContentCell_OSPD_namelength_merge();
        }else if(cellSelector==4){
            selector=new Sampler_LTableContentCell_OSPD_combined(stopWords);
        }else{
            selector=new Sampler_LTableContentCell_OSPD_random();
        }

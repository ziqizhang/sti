...
import uk.ac.shef.dcs.sti.xtractor.TableXtractorRottenTomato;
 ...
 */
public class Test_ISWC_TableInterpretation_RottenTomato {
    private static Logger log = Logger.getLogger(Test_ISWC_TableInterpretation_RottenTomato.class.getName());
        ...
        for (File f : all) {
            ...
            try {
                ...
                if (!complete) {
                    ...
                    try {
                        missedWriter = new PrintWriter(new FileWriter("ti_rt(iswc)_missed.csv", true));
                ...
                PrintWriter missedWriter = null;
                try {
                    missedWriter = new PrintWriter(new FileWriter("ti_rt_missed(iswc).csv", true));
                    missedWriter.println(count + "," + inFile);
                    missedWriter.close();
                } catch (IOException e1) {
                    e1.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
    ...
    public static boolean process(MainInterpreter_and_Forward_learner interpreter, LTable table, String sourceTableFile, LTableAnnotationWriter writer,
        ...
        try {
            ...
            write_iswc_output(table,annotations,new File(outFilename).getName(), "film-rt-actor.txt.tableminer");

...
public class LRDFa11Parser {
    ...
    // RDFa1.0[5.5.9.2]
    protected static Object[] getAsPlainLiteral(Node node, String currentLanguage) {
        final String content = DomUtils.readAttribute(node, CONTENT_ATTRIBUTE, null);
        if (content != null) {
            Object[] rs = new Object[2];
            rs[0] = RDFUtils.literal(content, currentLanguage);
            rs[1] = LAny23Util.getFullXPath(node) + LAny23Util.appendAttributeToXPath(CONTENT_ATTRIBUTE);
            return rs;
        }

        if (!node.hasChildNodes()) {
            Object[] rs = new Object[2];
            rs[0] = RDFUtils.literal("", currentLanguage);
            rs[1] = LAny23Util.getFullXPath(node);
        }
        ...
        if (nodeTextContent == null)
            return null;
        Object[] rs = new Object[2];
        rs[0] = RDFUtils.literal(nodeTextContent.trim(), currentLanguage);
        rs[1] = LAny23Util.getFullXPath(node) + "/text()";
        return rs;
    }
    ...
    protected static Object[] getAsXMLLiteral(Node node) throws IOException, TransformerException {
        ...
        Object[] rs = new Object[2];
        rs[0] = literal;
        rs[1] = LAny23Util.getFullXPath(node) + LAny23Util.appendAttributeToXPath(DATATYPE_ATTRIBUTE);
     ...
     * @param document
     */
    public void processDocument(URL documentURL, Document document, LExtractionResultImpl extractionResult)
     ...
     * @param extractionResult
     */
    private void depthFirstChildren(NodeList nodeList, LExtractionResultImpl extractionResult) {
     ...
     * @param extractionResult
     */
    private void writeTriple(Resource s, URI p, Value o, String sCtx, String pCtx, String oCtx, LExtractionResultImpl extractionResult) {
        ...
        extractionResult.writeTriple(s, p, o, sCtx, pCtx, oCtx);
    }
    ...
    private void processNode(Node currentElement, LExtractionResultImpl extractionResult) throws Exception {
        ...
        try {
            ...
            */
            ...
            for (URI type : types) {
                String objectCtx = LAny23Util.getFullXPath(currentElement) + LAny23Util.appendAttributeToXPath(TYPEOF_ATTRIBUTE);
                String subjectCtx = currentEvaluationContext.newSubjectCtx;

                writeTriple(currentEvaluationContext.newSubject, RDF.TYPE, type,
                        subjectCtx, null, objectCtx, extractionResult);
            }
            ...
            if (currentEvaluationContext.currentObjectResource != null) {
                for (URI rel : rels) {
                    String subjectCtx = currentEvaluationContext.newSubjectCtx;
                    String predicateCtx = LAny23Util.getFullXPath(currentElement) + LAny23Util.appendAttributeToXPath(REL_ATTRIBUTE);
                ...
                for (URI rev : revs) {
                    String objectCtx = currentEvaluationContext.currentObjectResourceCtx;
                    String subjectCtx = currentEvaluationContext.newSubjectCtx;
                    String predicateCtx = LAny23Util.getFullXPath(currentElement) + LAny23Util.appendAttributeToXPath(REV_ATTRIBUTE);
                    writeTriple(
                            currentEvaluationContext.currentObjectResource,
                            rev,
                            currentEvaluationContext.newSubject,
            ...
            final Object[] currentObject = getCurrentObject(currentElement);
            final URI[] predicates = getPredicate(currentElement);
            if (currentObject != null && predicates != null) {
                String objectCtx = currentObject[1].toString();
                String subjectCtx = currentEvaluationContext.newSubjectCtx;

                for (URI predicate : predicates) {
                    String predicateCtx = LAny23Util.getFullXPath(currentElement) + LAny23Util.appendAttributeToXPath(PROPERTY_ATTRIBUTE);
                    writeTriple(currentEvaluationContext.newSubject, predicate, (Value) currentObject[0],
                            subjectCtx, predicateCtx, objectCtx, extractionResult);
                }
            }
            ...
            if (currentEvaluationContext.recourse) {
                ...
                if (currentEvaluationContext.skipElem) {
                    ...
                    if (currentEvaluationContext.newSubject != null) {
                        ...
                        newEvaluationContext.parentSubjectCtx = currentEvaluationContext.newSubjectCtx;
                    } else {
                        newEvaluationContext.parentSubject = currentEvaluationContext.parentSubject;
                        newEvaluationContext.parentSubjectCtx = currentEvaluationContext.parentSubjectCtx;
                    }

                    if (currentEvaluationContext.currentObjectResource != null) {
                        newEvaluationContext.parentObject = currentEvaluationContext.currentObjectResource;
                        newEvaluationContext.parentObjectCtx = currentEvaluationContext.currentObjectResourceCtx;
                    } else if (currentEvaluationContext.newSubject != null) {
                        newEvaluationContext.parentObject = currentEvaluationContext.newSubject;
                        newEvaluationContext.parentObjectCtx = currentEvaluationContext.newSubjectCtx;
                    } else {
                        newEvaluationContext.parentObject = currentEvaluationContext.parentSubject;
                        newEvaluationContext.parentObjectCtx = currentEvaluationContext.parentSubjectCtx;
                    }
    ...
    private void establishNewSubject(Node node, EvaluationContext currentEvaluationContext)
        ...
        for (String subjectAttribute : SUBJECT_ATTRIBUTES) {
            ...
            if (candidateURIOrCURIE != null) {
                ...
                currentEvaluationContext.newSubjectCtx = LAny23Util.getFullXPath(node) + LAny23Util.appendAttributeToXPath(subjectAttribute);
                return;
            }
        }
        ...
        if (node.getNodeName().equalsIgnoreCase(HEAD_TAG) || node.getNodeName().equalsIgnoreCase(BODY_TAG)) {
            ...
            currentEvaluationContext.newSubjectCtx = LAny23Util.getFullXPath(node);
            currentEvaluationContext.newSubjectCtx = currentEvaluationContext.newSubjectCtx + docmentBaseCtx;
            return;
        }

        if (DomUtils.hasAttribute(node, TYPEOF_ATTRIBUTE)) {
            currentEvaluationContext.newSubject = RDFUtils.bnode();
            currentEvaluationContext.newSubjectCtx = LAny23Util.getFullXPath(node) + LAny23Util.appendAttributeToXPath(TYPEOF_ATTRIBUTE);
            return;
        }
        ...
        if (currentEvaluationContext.parentObject != null) {
            ...
            currentEvaluationContext.newSubjectCtx = currentEvaluationContext.parentObjectCtx;
            return;
        }
    ...
    private void establishNewSubjectCurrentObjectResource(Node node, EvaluationContext currentEvaluationContext)
        ...
        if (candidateURIOrCURIE != null) {
            currentEvaluationContext.newSubject = resolveCURIEOrURI(candidateURIOrCURIE, false);
            currentEvaluationContext.newSubjectCtx = LAny23Util.getFullXPath(node) + LAny23Util.appendAttributeToXPath(ABOUT_ATTRIBUTE);
        } else {
            ...
            if (candidateURIOrCURIE != null) {
                ...
                currentEvaluationContext.newSubjectCtx = LAny23Util.getFullXPath(node) + LAny23Util.appendAttributeToXPath(SRC_ATTRIBUTE);
            } else {
                if (node.getNodeName().equalsIgnoreCase(HEAD_TAG) || node.getNodeName().equalsIgnoreCase(BODY_TAG)) {
                    currentEvaluationContext.newSubject = resolveURI(currentEvaluationContext.base.toString());
                    currentEvaluationContext.newSubjectCtx = LAny23Util.getFullXPath(node);
        ...
        if (candidateURIOrCURIE != null) {
            ...
            currentEvaluationContext.currentObjectResourceCtx = LAny23Util.getFullXPath(node) + LAny23Util.appendAttributeToXPath(RESOURCE_ATTRIBUTE);
            return;
        }
        ...
        if (candidateURIOrCURIE != null) {
            ...
            currentEvaluationContext.currentObjectResourceCtx = LAny23Util.getFullXPath(node) + LAny23Util.appendAttributeToXPath(HREF_ATTRIBUTE);
            return;
        }
     ...
     */
    private Object[] getCurrentObject(Node node)
        ...
        if (candidateObject != null) {
            Object[] rs = new Object[2];
            rs[0] = resolveURI(candidateObject);
            rs[1] = LAny23Util.getFullXPath(node) + LAny23Util.appendAttributeToXPath(HREF_ATTRIBUTE);
            return rs;
        } else {
    ...
    private Object[] gerCurrentObjectLiteral(Node node)
            throws URISyntaxException, IOException, TransformerException {
        final EvaluationContext currentEvaluationContext = getContext();
        Object[] rs;
     ...
     * @throws URISyntaxException
     */
    private Object[] getAsTypedLiteral(Node node) throws URISyntaxException {
        final String datatype = DomUtils.readAttribute(node, DATATYPE_ATTRIBUTE, null);
        ...
        Literal literal = RDFUtils.literal(getNodeContent(node), curieOrURI instanceof URI ? (URI) curieOrURI : null);
        if (literal == null)
            return null;
